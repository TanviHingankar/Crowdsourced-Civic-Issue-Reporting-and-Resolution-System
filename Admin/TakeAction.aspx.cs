...
public partial class Admin_NewMess : System.Web.UI.Page
{
    ...
    private void LoadIssueDetails(int id)
    {
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM Issues WHERE IssueID=@id", con);
            ...
            if (dr.Read())
            {
                ...
                {
                    imgIssuePhoto.ImageUrl = "~/uploads/issues/" + dr["Photo"].ToString();
                }
            }
            dr.Close();
        }
    ...
    protected void btnSaveAction_Click(object sender, EventArgs e)
    {
        ...
        using (SqlConnection con = new SqlConnection(connStr))
        {
            ...
            SqlCommand cmd = new SqlCommand(
                "UPDATE Issues SET Status=@status, Remarks=@remarks, CompletionPhoto=@photo WHERE IssueID=@id", con);
